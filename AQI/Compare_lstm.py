...
class lstm(nn.Module):
    def __init__(self, input_size=2, hidden_size=4, output_size=1, num_layer=2, name='_lstm_'):
    ...
    def load(self):
        ...
        self.load_state_dict(state['lstm'])

    def save(self, epoch):
        state = {}
        state[epoch] = epoch
        state['lstm'] = self.state_dict()
...
if pred_type == 'local':
    ...
    model = lstm(input_size=n_his, hidden_size=16, output_size=1, num_layer=7, name='_lstm_'+pred_type).cuda()
...
if  pred_type == 'global':
    ...
    model = lstm(input_size=n_his, hidden_size=16, output_size=10, num_layer=7, name='_lstm_'+pred_type).cuda()
